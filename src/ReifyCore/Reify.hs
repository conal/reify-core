...
lamName = moduledName "ReifyCore.Lambda"
...
inReify :: Unop ReExpr
inReify = wrapReifyE <~ unReify

reifyIf :: ReExpr
reifyIf =
  unReify >>>
  do (Var (fqVarName -> "ReifyCore.Lambda.if'"),args@(length -> 2)) <- callT
...
-- reifyR :: ReCore
-- reifyR = anytdR (repeatR (promoteR reifyMisc))
...
externals =
    ...
    , externC "reify-oops" reifyOops "Generate errors for remaining reifyEP call"
    ]
